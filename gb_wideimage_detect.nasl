...
foreach dir( make_list_unique( "/", "/WideImage-master", "/wideimage", http_cgi_dirs( port:port ) ) ) {
  ...
  foreach url( make_list( "/index.php", "/doc/index.html", "/composer.json" ) ) {
    ...
    if( rcvRes =~ "^HTTP/1\.[01] 200" && ( "<title>WideImage" >< rcvRes || rcvRes =~ "homepage.*wideimage" ) ) {
