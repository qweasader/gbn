...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-3013-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-3013-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2014/DSA-3013-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-3013");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 's3ql' package(s) announced via the DSA-3013-1 advisory.");
...
if(release == "DEB7") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"s3ql", ver:"1.11.1-3+deb7u1", rls:"DEB7"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"s3ql-dbg", ver:"1.11.1-3+deb7u1", rls:"DEB7"))) {
    report += res;
  }
