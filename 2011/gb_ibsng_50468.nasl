...
if (description)
{
  ...
  script_name("IBSng XSS Vulnerability (Nov 2011) - Active Check");
  ...
  script_tag(name:"summary", value:"IBSng is prone to a cross-site scripting (XSS) vulnerability
...
foreach dir( make_list_unique( "/", http_cgi_dirs( port:port ) ) ) {
  ...
  res = http_get_cache( port:port, item:dir + "/IBSng/util/show_multistr.php" );
  if( res !~ "^HTTP/1\.[01] 200" || "<title>IBSng" >!< res )
    continue;

  url = dir + "/IBSng/util/show_multistr.php?str=%3Cscript%3Ealert(/ovas-xss-test/)%3C/script%3E";

  if( http_vuln_check( port:port, url:url, pattern:"<script>alert\(/ovas-xss-test/\)</script>",
                       check_header:TRUE, extra_check:"<title>IBSng" ) ) {
