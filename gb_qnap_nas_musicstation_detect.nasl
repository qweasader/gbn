...
if(description)
{
  ...
  script_name("QNAP Music Station Detection (HTTP)");

  script_tag(name:"summary", value:"HTTP based detection of QNAP Music Station.");
...
# nb: Music Station is part of QNAP QTS but we're checking all NAS OS variants just to be sure...
...
if ("<title>Music Station</title>" >< res && "QMS.Mime" >< res) {
  ...
  set_kb_item(name: "qnap/musicstation/detected", value: TRUE);
  set_kb_item(name: "qnap/musicstation/http/detected", value: TRUE);
  ...
  log_message(data: build_detection_report(app: "QNAP Music Station", version: version,
