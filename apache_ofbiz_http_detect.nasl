...
if(description)
{
  ...
  script_copyright("Copyright (C) 2009 Christian Eric Edjenguele / 2018 Greenbone Networks GmbH");
...
foreach url( make_list_unique( "/", default_modules, http_cgi_dirs( port:port ) ) ) {
  ...
  if( ( ofbizTitle && 'ofbiz' >< tolower( ofbizTitle[1] ) ) ||
    ...
    vers = eregmatch( pattern:'powered by <a href="https?://ofbiz\\.apache\\.org" target="_blank">[a-zA-Z ]+ ([0-9.]+)', string:res, icase:TRUE );
...
if( installed ) {
  ...
  set_kb_item( name:"apache/ofbiz/http/detected", value:TRUE );
