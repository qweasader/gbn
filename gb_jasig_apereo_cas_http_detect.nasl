...
if( found > 1 ) {
  ...
  if( version == "unknown" ) {
    ...
    if( res && res =~ "^HTTP/1\.[01] 401.+WWW-Authenticate\s*:.+" ) {
      ...
      if( ! user && ! pass ) {
        extra = "Apereo CAS and '/actuator/info' endpoint detected but version unknown. Providing credentials to the VT 'Jasig / Apereo Central Authentication Service (CAS) Detection (HTTP)' (OID: 1.3.6.1.4.1.25623.1.0.806501) might allow to gather the version.";
      } else if( ! user && pass ) {
