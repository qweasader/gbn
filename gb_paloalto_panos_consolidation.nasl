...
if(description)
{
  ...
  script_name("Palo Alto Device / PAN-OS Detection Consolidation");
  ...
  script_tag(name:"summary", value:"Consolidation of Palo Alto device detections including the
...
os_name = "Palo Alto PAN-OS";
os_cpe = "cpe:/o:paloaltonetworks:pan-os";
hw_app_name = "Palo Alto";
hw_app_cpe = "cpe:/h:paloaltonetworks";
...
if( detected_model != "unknown" ) {
  ...
  # nb: If we know that we have detected a VM we're using a cpe:/a: instead
  if( detected_model == "PA-VM" )
    hw_app_cpe = "cpe:/a:paloaltonetworks";

  hw_app_name += " " + detected_model;
  hw_app_cpe += ":" + tolower( detected_model );
} else {
  hw_app_name += " Unknown Model";
  hw_app_cpe += ":unknown_model";
}
...
if( os_version )
  os_register_and_report( os:"Palo Alto PAN-OS " + os_version, cpe:os_cpe, desc:"Palo Alto Device / PAN-OS Detection Consolidation", runs_key:"unixoide" );
else
  os_register_and_report( os:"Palo Alto PAN-OS", cpe:os_cpe, desc:"Palo Alto Device / PAN-OS Detection Consolidation", runs_key:"unixoide" );
...
if( xml_api_ports = get_kb_list( "palo_alto/xml-api/port" ) ) {
  foreach port( xml_api_ports ) {
    ...
    register_product( cpe:hw_app_cpe, location:location, port:port, service:"xml-api" );
...
if( ssh_ports = get_kb_list( "palo_alto/ssh-login/port" ) ) {
  foreach port( ssh_ports ) {
    ...
    register_product( cpe:hw_app_cpe, location:location, port:port, service:"ssh-login" );
...
report += '\n\n' + build_detection_report( app:hw_app_name,
                                           skip_version:TRUE,
                                           install:location,
                                           cpe:hw_app_cpe );
...
if( failed_reasons ) {
  report += '\n\nHTTP XML-API credentials were provided via "Palo Alto Device / PAN-OS Detection (HTTP XML-API)" ';
  report += '(OID:1.3.6.1.4.1.25623.1.0.105262) but the login at the HTTP XML-API failed for the following reasons:\n';
  report += '\n' + failed_reasons;
}
