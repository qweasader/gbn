...
if(description)
{
  ...
  script_name("CODESYS OPC-Server Detection (Windows SMB Login)");
...
foreach key (key_list) {
  foreach item (registry_enum_keys(key:key)) {
    ...
    register_and_report_cpe(app:appName, ver:version, concluded:concluded,
                          base:"cpe:/a:codesys:codesys_opc_server:", expr:"^([0-9.]+)", insloc:location, regService:"smb-login", regPort:0);
    exit(0);
  }
}
