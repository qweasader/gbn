...
if(description)
{
  ...
  script_tag(name:"summary", value:"HTTP based detection of OpenText FirstClass.");
...
if( banner =~ "Server\s*:\s*FirstClass" ) {
  ...
  ver = eregmatch( string: banner, pattern: "Server\s*:\s*FirstClass/([0-9.]+)", icase: TRUE );
  if( ! isnull( ver[1] ) ) {
