...
if(description)
{
  ...
  script_name("SolarWinds IP Address Manager (IPAM) Detection (Windows SMB Login)");
  ...
  script_tag(name:"summary", value:"SMB login-based detection of SolarWinds IP Address Manager
  (IPAM).");
...
foreach key(key_list) {
  foreach item(registry_enum_keys(key:key)) {
    ...
    if(!appName || appName !~ "SolarWinds( Orion)? IP Address Manager")
      continue;
    ...
    set_kb_item(name:"solarwinds/ipam/smb-login/detected", value:TRUE);
