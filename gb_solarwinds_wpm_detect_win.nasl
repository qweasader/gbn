...
if(description)
{
  ...
  script_name("SolarWinds Web Performance Monitor (WPM) Detection (Windows SMB Login)");
  ...
  script_tag(name:"summary", value:"SMB login-based detection of SolarWinds Web Performance Monitor
  (WPM).");
...
foreach key(key_list) {
  foreach item(registry_enum_keys(key:key)) {
    ...
    if(!appName || appName !~ "SolarWinds( Orion)? Web Performance Monitor")
      continue;
    ...
    set_kb_item(name:"solarwinds/wpm/smb-login/detected", value:TRUE);
