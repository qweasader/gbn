...
if(description)
{
  ...
  script_dependencies("ftpserver_detect_type_nd_version.nasl", "gb_starttls_ftp.nasl", "logins.nasl");
...
foreach user( keys( creds ) ) {
  ...
  # nb: Check again if something changed during the scan (e.g. account blocked, service throwing a 500, ...) after the initial grabbing of the banner via ftp_get_banner().
  if( banner !~ "^2[0-9]{2}[ -].+" ) {
