...
foreach dir( make_list_unique( "/", "/docushare", "/share", http_cgi_dirs( port:port ) ) ) {
  ...
  if( found > 1 ) {
    ...
    url = dir + "/dsweb/webex/webExMeetingLogin.jsp";
    res = http_get_cache( item:url, port:port );
    if( res && res =~ "^HTTP/1\.[01] 200" &&
        # nb: Keep the pattern in "sync" with the related active XSS VT
        ( ">WebEx Login Page<" >< res || "<title>Meet Online</title>" >< res || "<!--File name:      webExMeetingLogin.jsp" >< res ) ) {
      extra = "- webEx Module available at: " + http_report_vuln_url( port:port, url:url, url_only:TRUE );
      set_kb_item( name:"xerox/docushare/modules/webex/detected", value:TRUE );
      set_kb_item( name:"xerox/docushare/modules/webex/http/detected", value:TRUE );
    }
