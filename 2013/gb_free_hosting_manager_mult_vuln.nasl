...
if(description)
{
  ...
  script_name("Free Hosting Manager <= 2.0.2 Multiple Vulnerabilities");
...
foreach dir(make_list_unique("/", "/freehostingmanager", "/fhm", http_cgi_dirs(port:port))) {
  ...
  if(http_vuln_check(port:port, url:url, check_header:TRUE, pattern:"<title>.*:.* - Advanced Package Details",
