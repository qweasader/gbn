...
if(description)
{
  ...
  script_dependencies("gb_kyocera_printer_consolidation.nasl", "gb_default_credentials_options.nasl");
  script_require_ports("Services/www", 80);
  script_mandatory_keys("kyocera/printer/http/detected");
  script_exclude_keys("default_credentials/disable_default_account_checks");

  script_tag(name:"summary", value:"The remote Kyocera Printer is using default or no credentials
  for the HTTP based interface.");
  ...
  script_tag(name:"affected", value:"All Kyocera printers using default or no credentials for the
...
mod = eregmatch(pattern: "^cpe:/o:kyocera:(.+)_firmware", string: cpe);
if (isnull(mod[1]))
  exit(0);
else
  model = mod[1];

ret = kyocera_check_default_login(model: model, port: port);

if (ret) {
  if (ret == 1) {
    message = "It was possible to login into the remote Kyocera " + model + ' printer with user "' +
              ky_last_user + '" and password "' + ky_last_pass;
  }
  else if (ret == 2) {
    message = "The remote Kyocera ' + model + ' printer is not protected by a username and password.";
  }
