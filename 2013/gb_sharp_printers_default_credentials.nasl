...
if(description)
{
  ...
  script_name("SHARP Printer Default / No Credentials (HTTP)");

  script_category(ACT_ATTACK);

  script_family("Default Accounts");
  script_copyright("Copyright (C) 2013 Greenbone Networks GmbH");
  script_dependencies("gb_sharp_printer_consolidation.nasl", "gb_default_credentials_options.nasl");
  script_require_ports("Services/www", 80);
  script_mandatory_keys("sharp/printer/http/detected");
  script_exclude_keys("default_credentials/disable_default_account_checks");

  script_tag(name:"summary", value:"The remote SHARP Printer is using default or no credentials for
  ...
  script_tag(name:"affected", value:"All SHARP printers using default or no credentials for the
...
if( ! model = get_kb_item( "sharp/printer/http/" + port + "/model" ) )
  exit( 0 );
...
if( ret ) {
  if( ret == 1 ) {
    message = "It was possible to login into the remote SHARP " + model + ' printer with user "' +
              sharp_last_user + '" and password "' + sharp_last_pass + '"';
  }
  else if( ret == 2 ) {
    message = "The remote SHARP " + model + " printer is not protected by a username and password.";
  }
