...
if( found ) {
  ...
  # nb: Generic KB key for "active" VTs which should check all Web Server variants
  ...
  if( ( res =~ "^HTTP/1\.[01] 200" && "Sun Java System Web Server" >< res ) || is_sun_java_system ) {
    ...
    if( version == "unknown" ) {
      # Server: Sun-Java-System-Web-Server/7.0
      # nb: This has only the minor version without the patch level like exposed at the admingui URL.
    ...
    log_message( data:build_detection_report( app:"Sun Java System Web Server",
                                              version:version,
                                              install:install,
                                              cpe:cpe,
  ...
  if( ( res =~ "^HTTP/1\.[01] 200" && "Oracle iPlanet Web Server" >< res ) || is_oracle_iplanet ) {

    version = "unknown";

    # Oracle iPlanet Web Server 7.0.24 B09/14/2016 09:03
