...
# 3 Phase step
# 1. Get interfaces with is-is neighbor relationship established
...
if(get_kb_item("Policy/vrp/installed/ERROR")){
  ...
  foreach interface(interfaces){
    ...
    if(ret !~ "isis authentication-mode" || ret =~ "isis authentication-mode md5 cipher"){
      value = "IS-IS neighbor relationship enabled, isis authentication-mode not secure";
      compliant = "no";
      comment += interface + " ";
    }
  }
  if(!value){
    value = "IS-IS neighbor relationship enabled, isis authentication-mode secure";
    compliant = "yes";
