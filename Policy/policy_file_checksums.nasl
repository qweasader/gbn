...
if(description)
{
  ...
  script_xref(name:"URL", value:"https://docs.greenbone.net/GSM-Manual/gos-22.04/en/compliance-and-special-scans.html#checking-file-checksums");
...
checksumlist = script_get_preference( "Target checksum File", id:1 );
if( ! checksumlist ) exit( 0 );

checksumlist = script_get_preference_file_content( "Target checksum File", id:1 );
if( ! checksumlist ) exit( 0 );
...
listall = script_get_preference("List all and not only the first 100 entries", id:2);
maxlist = 100;
host_ip = get_host_ip();
