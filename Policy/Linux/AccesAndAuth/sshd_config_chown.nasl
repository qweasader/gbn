...
if(description)
{
  ...
  script_tag(name:"summary", value:"The /etc/ssh/sshd_config file contains configuration specifications for
  sshd. This should be protected from unauthorized changes by non-privileged users.");
...
solution = "chown root:root /etc/ssh/sshd_config
...
if(get_kb_item("Policy/linux/sshd_config/stat/ERROR")){
  ...
  comment = "Cannot determine configuration in /etc/ssh/sshd_config";
}else{
