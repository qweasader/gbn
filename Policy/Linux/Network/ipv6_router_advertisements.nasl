...
if(description)
{
  ...
  script_dependencies("compliance_tests.nasl", "read_and_parse_sysctl.nasl", "read_etc_sysctl_d.nasl");
...
if(get_kb_item("Policy/linux/sysctl/conf/ERROR") ||
  ...
  foreach content (content_list){
    if(content =~ "net\.ipv6\.conf\.all\.accept_ra\s*=\s*1" ||
       content =~ "net\.ipv6\.conf\.default\.accept_ra\s*=\s*1"){
      value = "Enabled";
    }
  }
  ...
  if( net_ipv6_conf_all_accept_ra != "0" ||
