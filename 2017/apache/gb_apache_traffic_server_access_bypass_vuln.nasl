...
if (description)
{
  ...
  script_name("Apache Traffic Server (ATS) 5.1.x < 5.1.1 Access Bypass Vulnerability");

  script_tag(name:"summary", value:"Apache Traffic Server is prone to an access bypass
  vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The flaw exists due to a failure to properly tunnel remap
  requests using CONNECT.");

  script_tag(name:"impact", value:"Successful exploitation will allow remote attackers to bypass
  access restrictions.");

  script_tag(name:"affected", value:"Apache Traffic Server version 5.1.x before 5.1.1.");
...
if (version =~ "^5\.1" && version_is_less(version: version, test_version: "5.1.1")) {
