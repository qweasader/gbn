...
if (description)
{
  ...
  script_name("Cisco NX-OS Software Fibre Channel over Ethernet Denial of Service Vulnerability (cisco-sa-20170607-nxos)");
  ...
  script_tag(name:"summary", value:"A vulnerability in the Fibre Channel over Ethernet (FCoE)
  protocol implementation in Cisco NX-OS Software could allow an unauthenticated, adjacent attacker
  to cause a denial of service (DoS) condition when an FCoE-related process unexpectedly reloads.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The vulnerability is due to a lack of proper FCoE frame padding
  validation. An attacker could exploit this vulnerability by sending a stream of crafted FCoE
  frames to the targeted device. The attacker's server must be directly connected to the FCoE
  interface on the device that is running Cisco NX-OS Software to exploit this vulnerability.");

  script_tag(name:"impact", value:"An exploit could allow the attacker to cause a DoS condition,
  which would impact FCoE traffic passing through the device.");
