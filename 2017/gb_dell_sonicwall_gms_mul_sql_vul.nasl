...
if(description)
{
  ...
  script_name("SonicWALL GMS / Analyzer 8.x < 8.2 Multiple SQLi Vulnerabilities");

  script_category(ACT_GATHER_INFO);

  script_copyright("Copyright (C) 2017 Greenbone Networks GmbH");
  script_family("Web application abuses");
  script_dependencies("gb_sonicwall_ums_gms_analyzer_http_detect.nasl");
  script_mandatory_keys("sonicwall/ums/detected");

  script_tag(name:"summary", value:"SonicWALL GMS / Analyzer are prone to multiple SQL injection
  (SQLi) vulnerabilities.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"SonicWALL GMS/Analyzer suffers from multiple SQLi
  vulnerabilities because it does not sufficiently sanitize user-supplied input.");

  script_tag(name:"impact", value:"A successful exploit may allow an attacker to compromise the
  application, access or modify data, or exploit latent vulnerabilities in the underlying database.");

  script_tag(name:"affected", value:"SonicWALL GMS/Analyzer version 8.0.x and 8.1.x.");
