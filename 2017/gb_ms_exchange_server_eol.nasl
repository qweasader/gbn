...
if(description)
{
  ...
  script_dependencies("gb_ms_exchange_server_detect.nasl", "gb_owa_detect.nasl");
...
# nb: gb_owa_detect.nasl is also setting this CPE but currently with a different version
# scheme. This wouldn't match the versions of the Exchange CPE as used in products_eol.inc
# so we need to skip this CPE for now. This is done by exiting here if port > 0 because
# the local detections of Exchange are setting the port as "0".
