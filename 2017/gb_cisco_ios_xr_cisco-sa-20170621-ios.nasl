...
if (description)
{
  ...
  script_name("Cisco IOS XR Software Local Command Injection Vulnerability (cisco-sa-20170621-ios)");
  ...
  an authenticated, local attacker to execute arbitrary commands on the host operating system with
  root privileges.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The vulnerability is due to insufficient input validation. An
  attacker could exploit this vulnerability by sending crafted input to a command in a specific
  group.");
