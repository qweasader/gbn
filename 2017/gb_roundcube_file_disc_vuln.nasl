...
if (description)
{
  ...
  script_name("Roundcube Webmail < 1.1.10, 1.2.x < 1.2.7, 1.3.x < 1.3.3 File Disclosure Vulnerability");
  ...
  script_tag(name:"summary", value:"Roundcube Webmail is prone to a file disclosure
  vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"Roundcube Webmail allows unauthorized access to arbitrary files
  on the host's filesystem, including configuration files. The attacker must be able to authenticate
  at the target system with a valid username/password as the attack requires an active session.

  The issue is related to file-based attachment plugins and
  _task=settings&_action=upload-display&_from=timezone requests.");

  script_tag(name:"affected", value:"Roundcube Webmail before 1.1.10, 1.2.x before 1.2.7 and 1.3.x
  before 1.3.3.");
...
if (version_is_less(version: version, test_version: "1.1.10")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "1.1.10", install_path: location);
