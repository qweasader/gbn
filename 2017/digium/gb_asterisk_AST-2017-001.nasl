...
if (description)
{
  ...
  script_name("Asterisk CDR Buffer Overflow Vulnerability (AST-2017-001)");
  ...
  script_tag(name:"summary", value:"Asterisk is prone to a buffer overflow vulnerability in CDR's set
  user.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"No size checking is done when setting the user field on a CDR.
  Thus, it is possible for someone to use an arbitrarily large string and write past the end of the
  user field storage buffer. This allows the possibility of remote code injection.");
  ...
  script_tag(name:"solution", value:"Update to version 13.14.1, 14.3.1, 13.13-cert3 or later.");
