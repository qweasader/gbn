...
if( ! portmap = get_kb_item( "rpc/portmap/port" ) )
  exit( 0 );

if( ! get_port_state( portmap ) )
  exit( 0 );

if( ! soc = open_sock_tcp( portmap ) ) # TODO: Won't work if the port is only available via UDP...
  exit( 0 );
...
while (vf)
{
  ...
  #if ((proto == 17 ) || get_port_state(port))
  {
    ...
    if (proto == 6)
    {
      ...
      {
       if (name)
         service_register(port: port, proto: string("RPC/", name));
       else
         service_register(port: port, proto: string("RPC/", program));
      }
    }
...
log_message(port:portmap, data:chomp(result));
exit(0);
