...
if( ! full_path_list = ssh_find_file( file_name:"/(pom\.xml|log4j(-core)?(-[0-9.x-]+)?\.jar)$", sock:sock, useregex:TRUE ) ) {
...
foreach full_path( full_path_list ) {
  ...
  if( "/pom.xml" >< full_path ) {
    ...
    # nb: If we didn't get the version from the file name or only a short one like "1.2"
    # we're trying to get it from a possible existing .pom file (exists on e.g. Debian).
