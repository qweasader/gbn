...
if (description)
{
  ...
  script_name("Cisco ASA Management Interface XML Parser DoS Vulnerability (cisco-sa-20151123-asa)");
  ...
  parser code. Cisco ASA may process a crafted XML file if the file is passed through the
  management interface or when performing activities with the auto update server AUS. In all cases
  a valid authentication on the device or a valid AUS server would need to be used in order to
  provide an XML file.");
  ...
  script_tag(name:"affected", value:"Cisco ASA version 7.2, 8.0, 8.1, 8.2, 8.3, 8.4, 8.5, 8.6, 8.7,
  9.0, 9.1, 9.2 and 9.3.");
