...
if(description)
{
  ...
  script_name("MediaWiki Hovercards Extension < 1.24 XSS Vulnerability (Jan 2015) - Active Check");
  ...
  script_tag(name:"summary", value:"The Hovercards extension for MediaWiki is prone to a cross-site
  scripting (XSS) vulnerability.");

  script_tag(name:"vuldetect", value:"Sends a crafted HTTP POST request and checks the response.");

  script_tag(name:"insight", value:"The flaw exists as input passed via text parameter to the
  'Extension:Popups' script is not validated before returning it to users.");
  ...
  script_tag(name:"affected", value:"The Hovercards extension for Mediawiki in versions prior to
  1.24.");

  script_tag(name:"solution", value:"Update the Hovercards extension to version 1.24 or later.");
...
if(res =~ ">Popups<") {
  ...
  if(res =~ "^HTTP/1\.[01] 200" && "javascript:alert(document.cookie)" >< res) {
