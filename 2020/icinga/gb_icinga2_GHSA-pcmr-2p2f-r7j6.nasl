...
if(description)
{
  ...
  script_tag(name:"summary", value:"Icinga 2 is prone to a vulnerability where revoked certificates
  are automatically renewed ignoring the CRL.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"Revoked certificates due for renewal will automatically be
  renewed ignoring the CRL.

  When a CRL is specified in the ApiListener configuration, Icinga 2 only used it when connections
  were established so far, but not when a certificate is requested. This allows a node to
  automatically renew a revoked certificate if it meets the other conditions for auto renewal
  (issued before 2017 or expires in less than 30 days).

  Because Icinga 2 currently uses a validity duration of 15 years, this only affects setups with
  external certificate signing and revoked certificates that expire in less then 30 days.");

  script_tag(name:"affected", value:"Icinga versions 2.8.0 through 2.11.7 and 2.12.2.");
