...
if(description)
{
  ...
  script_dependencies("gb_plex_media_server_http_detect.nasl", "os_detection.nasl");
  script_mandatory_keys("plex_media_server/detected", "Host/runs_windows");

  script_tag(name:"summary", value:"Plex Media Server is prone to a local unauthenticated code
  execution vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"Improper Input Validation in Plex Media Server on Windows allows
  a local, unauthenticated attacker to execute arbitrary Python code with SYSTEM privileges.");
