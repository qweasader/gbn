...
if (description)
{
  ...
  script_tag(name:"summary", value:"The WordPress plugin 'Popup Builder' is prone to an SQL injection
  vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The plugin is vulnerable to SQL injection via PHP deserialization
  on attacker-controlled data with the attachmentUrl POST variable. This allows the creation of an
  arbitrary WordPress Administrator account, leading to possible remote code execution because
  administrators can run PHP code on WordPress instances.");

  script_tag(name:"impact", value:"Successful exploitation of this vulnerability would allow a remote
  attacker to execute arbitrary SQL commands on the affected system.");
