...
if(description)
{
  ...
  script_name("Burp Suite Community Edition Detection (Windows SMB Login)");
...
foreach key( key_list ) {
  foreach item( registry_enum_keys( key:key ) ) {
    ...
    if( ! appName || appName !~ "Burp Suite Community Edition" )
      continue;
    ...
    register_and_report_cpe( app:appName, ver:version, concluded:concluded,
                             base:"cpe:/a:portswigger:burp_suite:", expr:"^([0-9.]+)", insloc:location, regService:"smb-login", regPort:0 );
    exit( 0 );
  }
}
