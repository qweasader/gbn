...
if(description)
{
  ...
  script_name("Splunk Enterprise / Free Detection (HTTP)");
  ...
  script_tag(name:"summary", value:"HTTP based detection Splunk Enterprise / Free.");
...
foreach dir (make_list_unique("/", "/splunk/en-US", "/en-US", http_cgi_dirs(port: port))) {
  ...
  if (egrep(pattern: 'content="Splunk Inc."', string: res1, icase: TRUE) &&
      (res1 =~ "Splunk (Enterprise|Free)" || res1 =~ 'product_type"\\s*:\\s*"(splunk|enterprise)')) {
    version = "unknown";
    ...
    if (os_name = eregmatch(string: res1, pattern:'"os_name"\\s*:\\s*"[^"]+"', icase: FALSE)) {
      ...
      banner_type = "Splunk Enterprise / Free 'os_name' Page Content";

      if (os_name =~ "Linux")
        os_register_and_report(os: "Linux", cpe: "cpe:/o:linux:kernel", port: port, banner_type: banner_type, banner: os_name[0], desc: "Splunk Enterprise / Free Detection (HTTP)", runs_key: "unixoide");
      else if (os_name =~ "Windows")
        os_register_and_report(os: "Windows", cpe: "cpe:/o:microsoft:windows", port: port, banner_type: banner_type, banner: os_name[0], desc: "Splunk Enterprise / Free Detection (HTTP)", runs_key: "windows");
      else
    ...
    log_message(data: build_detection_report(app: "Splunk Enterprise / Free", version: version, build: build,
