...
if(description)
{
  ...
  script_tag(name:"insight", value:"It is possible to persistently inject scripts in XWiki.
  Unregistered users can fill simple text fields. Registered users can fill in their personal
  information and (if they have edit  rights) fill the values of static lists using App Within
  Minutes.");

  script_tag(name:"impact", value:"Successful exploitation can lead to user's session hijacking, and
  if used in conjunction with a social engineering attack it can also lead to disclosure of
  sensitive data, CSRF attacks and other security vulnerabilities. That can also lead to the
  attacker taking over an account. If the victim has administrative rights it might even lead to
  code execution on the server, depending on the application and the privileges of the account.");

  script_tag(name:"affected", value:"XWiki versions prior to 12.6.3 or 12.8.");
