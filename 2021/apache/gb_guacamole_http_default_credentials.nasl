...
if(description)
{
  ...
  script_version("2022-02-15T15:02:34+0000");
  script_tag(name:"last_modification", value:"2022-02-15 15:02:34 +0000 (Tue, 15 Feb 2022)");
...
url = dir + "/api/tokens";
report = "It was possible to gather a valid 'authToken' with the following known default credentials (username:password:authToken) via a login at '" + http_report_vuln_url( port:port, url:url, url_only:TRUE ) + "':" + '\n';
...
foreach username( keys( creds ) ) {
  ...
  if( ! headers || ! body || headers !~ "Content-Type\s*:\s*application/json" )
    continue;

  # e.g. the following with a 200 status code for successful login:
  ...
  # or the following on invalid credentials with a 403 status code:
  ...
  # see e.g.:
  # https://github.com/ridvanaltun/guacamole-rest-api-documentation/blob/master/docs/AUTHENTICATION.md#generate-token
  if( egrep( string:body, pattern:'"username"\\s*:\\s*"' + username + '"', icase:FALSE ) &&
      egrep( string:body, pattern:'"authToken"\\s*:\\s*"[^"]+"', icase:FALSE ) ) {

    token = "N/A";
    to = eregmatch( string:body, pattern:'"authToken"\\s*:\\s*"([^"]+)"', icase:FALSE );
    if( to[1] )
      token = to[1];
