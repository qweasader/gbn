...
if (description)
{
  ...
  script_version("2021-12-03T08:27:06+0000");
  script_tag(name:"last_modification", value:"2021-12-03 08:27:06 +0000 (Fri, 03 Dec 2021)");
  script_tag(name:"creation_date", value:"2021-11-24 07:35:11 +0000 (Wed, 24 Nov 2021)");
  # nb: CVE linked below has a severity but is very broad, using a severity would cause too many
  # false positives as we're not able to determine if a system has IP Forwarding enabled on purpose
  # (e.g. a Router/Firewall) or not. This needs to be inspected manually.
...
# nb: Some systems (have seen this on a Palo Alto PA-220 doesn't forward the crafted TCP request but
# using an ICMP ECHO request sent to an external system works so we're checking both cases just to be sure.
