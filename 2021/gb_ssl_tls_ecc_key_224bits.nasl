...
check_algorithms = "^(EC/ECDSA|EdDSA \(Ed25519\)|EdDSA \(Ed448\)|ECDH \(X25519\)|ECDH \(X448\))$";
...
if( report ) {
  security_message( port:port, data:'The remote SSL/TLS server is using the following certificate(s) with a ECC key with less than ' + min_key_size + ' bits (public-key-size:public-key-algorithm:serial:issuer):\n' + report );
  exit( 0 );
}
