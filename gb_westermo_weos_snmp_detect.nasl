...
if (description)
{
  ...
  script_tag(name:"summary", value:"SNMP based detection of Westermo WeOS devices like e.g. Westermo
  Falcon, Westermo Lynx or Westermo RedFox.");
...
# Westermo RedFox, primary: v4.33.2, backup: v4.33.2, bootloader: v2017.12.0-10, fpga: v40010504
# Westermo RedFox, primary: v4.15.3, backup: v0.00, bootloader: v5.00, fpga: v40010503
if (egrep(string: sysdesc, pattern: "^Westermo.*, primary:.*, backup:.*, bootloader:")) {
  ...
  if (!isnull(mo[1])) {
    ...
    if (!mod || mod == "") {
      # nb: Wait a few seconds as some devices might be slow to respond on subsequent queries
      sleep(2);
      # nb: Falcon
    ...
    if (!mod || mod == "") {
      ...
      sleep(2);
      # nb: RedFox
      mod_oid = "1.3.6.1.2.1.47.1.1.1.1.13.6";
      mod = snmp_get(port: port, oid: mod_oid);
    }
