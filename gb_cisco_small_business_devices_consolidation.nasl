...
if (description)
{
  ...
  script_name("Cisco Small Business Device Detection Consolidation");
...
foreach source (make_list("snmp", "http")) {
  ...
  foreach model (model_list) {
    if (model != "unknown" && detected_model == "unknown") {
      ...
      set_kb_item(name: "cisco/small_business/model", value: detected_model);
      break;
    }
  }
...
os_register_and_report(os: os_name, cpe: os_cpe, runs_key: "unixoide",
                       desc: "Cisco Small Business Device Detection Consolidation");
