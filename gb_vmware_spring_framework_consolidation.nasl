...
if(description)
{
  ...
  script_version("2022-04-11T13:11:11+0000");
  script_tag(name:"last_modification", value:"2022-04-11 13:11:11 +0000 (Mon, 11 Apr 2022)");
  ...
  script_dependencies("gb_vmware_spring_framework_ssh_login_detect.nasl",
                      "gb_vmware_spring_framework_smb_login_detect.nasl");
  ...
  foreach install( install_list ) {
    ...
    if( ! comp ) {
      ...
      if( comp == "core" ) {
        ...
        # nb: In the NVD a lower dash between product and component (e.g. spring_cloud_gateway) is
        # used so we're doing the same here BUT only for the CPE and not at other places like the
        # component name in the reporting or the KB keys in the other detection VTs.
