...
if (description)
{
  ...
  script_name("SolarWinds Network Configuration Manager (NCM) Detection (HTTP)");
  ...
  script_tag(name:"summary", value:"HTTP based detection of SolarWinds Network Configuration Manager
  (NCM).");
...
if ("SolarWinds Platform" >< buf || "SolarWinds Orion" >< buf || "Orion Platform" >< buf) {
  ...
  if (!isnull(ncm)) {
    ...
    set_kb_item(name:"solarwinds/ncm/http/detected", value:TRUE);
    ...
    log_message(data:build_detection_report(app:"SolarWinds Network Configuration Manager (NCM)", version:vers,
                                            install:dir, cpe:cpe,
                                            concluded:ncm[0]),
                port:port);
  }
