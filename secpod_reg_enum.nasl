...
smbSessInfo = smb_login_and_get_tid_uid(soc:soc, name:name, login:infos["username_plain"], passwd:infos["password"], domain:infos["domain"], share:"IPC$");

if(isnull(smbSessInfo)) {
  close(soc);
  exit(0);
}
