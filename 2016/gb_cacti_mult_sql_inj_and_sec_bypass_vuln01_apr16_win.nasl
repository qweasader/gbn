...
if(description)
{
  ...
  script_name("Cacti <= 0.8.8f Multiple Vulnerabilities - Windows");
  ...
  - An insufficient validation of user supplied input via parameter 'cg_g' in the host_new_graphs
  function of the graphs_new.php script.

  - An insufficient validation of user supplied input via parameter 'rra_id' in a properties action
  to the graph.php script.
  ...
  arbitrary SQL commands and to bypass intended access restrictions.");

  script_tag(name:"affected", value:"Cacti version 0.8.8f and prior.");

  script_tag(name:"solution", value:"Update to version 0.8.8g or later.");
...
if(version_is_less_equal(version:vers, test_version:"0.8.8f")) {
  report = report_fixed_ver(installed_version:vers, fixed_version:"0.8.8g");
