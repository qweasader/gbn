...
if(description)
{
  ...
  script_name("IBM WebSphere Application Server Multiple Vulnerabilities (swg21682767, swg21671835)");
  ...
  script_tag(name:"insight", value:"Multiple flaws exist due to improper validation of input in the
  Administrative Console.");

  script_tag(name:"impact", value:"Successful exploitation will allow a remote attacker to obtain
  sensitive information, perform cross-site scripting attacks, perform session injection and other
  malicious activities, also to inject script into a victim's Web browser within the security
  context of the hosting Web site.");

  script_tag(name:"affected", value:"IBM WebSphere Application Server version 6.x through 6.1.0.47,
  7.0.x prior to 7.0.0.35, 8.0.x prior to 8.0.0.10 and 8.5.x prior to 8.5.5.4.");

  script_tag(name:"solution", value:"Update to version 7.0.0.35, 8.0.0.10, 8.5.5.4 or later.
...
if (version_in_range(version: version, test_version: "6.0", test_version2: "6.1.0.47")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "See advisory");
  security_message(port: 0, data: report);
  exit(0);
}

if (version_in_range_exclusive(version: version, test_version_lo: "7.0", test_version_up: "7.0.0.35")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "7.0.0.35");
  security_message(port: 0, data: report);
  exit(0);
}
