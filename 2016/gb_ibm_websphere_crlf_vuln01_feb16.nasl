...
if(description)
{
  ...
  script_name("IBM WebSphere Application Server CRLF Injection Vulnerability (Feb 2016)");
  ...
  script_tag(name:"summary", value:"IBM WebSphere Application Server is prone to a CRLF injection
  vulnerability.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The flaw is due to an HTTP response splitting attack
  vulnerability.");

  script_tag(name:"impact", value:"Successful exploitation will allow a remote attacker to use
  specially-crafted URL to cause the server to return a split response, once the URL is clicked.
  This would allow the attacker to perform further attacks, such as Web cache poisoning, cross-site
  scripting, and possibly obtain sensitive information.");

  script_tag(name:"affected", value:"IBM WebSphere Application Server version 6.1.x through
  6.1.0.47, 7.0.x prior to 7.0.0.39, 8.0.x prior to 8.0.0.12 and 8.5.x prior to 8.5.5.8.");

  script_tag(name:"solution", value:"Update to version 6.1.0.48, 7.0.0.39, 8.0.0.12, 8.5.5.8 or
  later.");
