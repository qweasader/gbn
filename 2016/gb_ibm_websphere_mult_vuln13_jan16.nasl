...
if(description)
{
  ...
  - Liberty Profile does not properly validate authentication cookies, when SSL is not enabled.

  - improper validation of user supplied input by the RPCAdapter for the Web2.0 and Mobile
  toolkit");

  script_tag(name:"impact", value:"Successful exploitation will allow remote attacker to bypass
  intended access restrictions, to inject arbitrary web script or HTML via a crafted response.");

  script_tag(name:"affected", value:"IBM WebSphere Application Server version 8.5.x prior to
  8.5.0.2.");

  script_tag(name:"solution", value:"Update to version 8.5.0.2 or later.");
