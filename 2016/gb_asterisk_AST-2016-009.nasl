...
if (description)
{
  ...
  script_name("Asterisk Authentication Bypass Vulnerability (AST-2016-009)");
  ...
  script_tag(name:"insight", value:"The chan_sip channel driver has a liberal definition for
  whitespace when attempting to strip the content between a SIP header name and a colon character.
  Headers such as 'Contact\x01:' will be seen as a valid Contact header.

  This mostly does not pose a problem until Asterisk is placed in tandem with an authenticating SIP
  proxy. In such a case, a crafty combination of valid and invalid To headers can cause a proxy to
  allow an INVITE request into Asterisk without authentication since it believes the request is an
  in-dialog request. However, because of the bug described above, the request will look like an
  out-of-dialog request to Asterisk. Asterisk will then process the request as a new call. The result
  is that Asterisk can process calls from unvetted sources without any authentication.

  If you do not use a proxy for authentication, or if your proxy is dialog-aware, or if you use
  chan_pjsip instead of chan_sip, then this issue does not affect you.");

  script_tag(name:"impact", value:"An authenticated remote attacker may make calls without any
  authentication.");

  script_tag(name:"affected", value:"Asterisk Open Source versions 11.x, 13.x, 14.x and Certified
  Asterisk versions 11.6 and 13.8.");

  script_tag(name:"solution", value:"Update to version 11.25.1, 13.13.1, 14.2.1, 11.6-cert16,
  13.8-cert4 or later.");
