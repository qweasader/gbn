...
if(description)
{
  ...
  script_tag(name:"summary", value:"Two security vulnerabilities have been identified with certain
  HP Inkjet printers. A maliciously crafted file sent to an affected device can cause a stack or
  static buffer overflow, which could allow remote code execution (RCE).");

  script_tag(name:"vuldetect", value:"The script checks if the target host is a vulnerable device
  running a vulnerable firmware version.");

  script_tag(name:"affected", value:"Multiple HP PageWide Pro, HP DesignJet, HP Officejet, HP
  Deskjet and HP Envy devices. See the referenced advisory for an extended list.");
