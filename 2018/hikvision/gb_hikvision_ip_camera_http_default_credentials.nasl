...
if(description)
{
  ...
  script_name("Hikvision IP Camera Default Credentials (HTTP)");
  script_dependencies("gb_hikvision_ip_camera_http_detect.nasl", "gb_default_credentials_options.nasl");
  ...
  script_tag(name:"summary", value:"The remote Hikvision IP camera device is using known default
  credentials.");
  ...
  script_tag(name:"vuldetect", value:"Checks if a successful login via HTTP to the Hikvision IP
  camera device is possible.");
...
foreach cred(keys(creds)) {
  ...
  #Digest authentication:
    ...
    info = eregmatch(pattern: 'WWW-Authenticate\\s*:\\s*Digest realm="([^"]+)",\\s*domain="[^"]+",\\s*qop="([^"]+)",\\s*nonce="([^"]+)",\\s*opaque="",\\s*algorithm="MD5"', string: res1);
    if(!isnull(info[1]) && !isnull(info[2]) && !isnull(info[3])) {
      realm = info[1];
      qop = info[2];
      nonce = info[3];
      ...
      info = eregmatch(pattern: 'WWW-Authenticate\\s*:\\s*Digest qop="([^"]+)",\\s*realm="([^"]+)",\\s*nonce="([^"]+)",', string: res1);
      if(isnull(info[1]) || isnull(info[2]) || isnull(info[3]))
        continue;
