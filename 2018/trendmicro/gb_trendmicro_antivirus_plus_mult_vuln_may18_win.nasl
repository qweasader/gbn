...
if(description)
{
  ...
  # nb: Patched version is not available from registry or anywhere, so it can result in FP for 12.0 patched versions
  script_tag(name:"qod", value:"30");

  script_name("Trend Micro Antivirus Plus < 12.0.1226 Multiple Vulnerabilities");

  script_tag(name:"summary", value:"Trend Micro Antivirus Plus is prone to multiple
  vulnerabilities.");
  ...
  - Processing of request ID 0x2002 for IDAMSPMASTER in the service process coreServiceShell.exe");

  script_tag(name:"impact", value:"Successful exploitation will allow attackers to escalate
  privileges, disclose sensitive information and inject malicious code into other processes.");

  script_tag(name:"affected", value:"Trend Micro Antivirus Plus 12.0 and prior.");

  script_tag(name:"solution", value:"Update to version 12.0.1226 or later.");
...
if(version_is_less_equal(version:vers, test_version:"12.0")) {
  report = report_fixed_ver(installed_version:vers, fixed_version:"12.0.1226", install_path:path);
