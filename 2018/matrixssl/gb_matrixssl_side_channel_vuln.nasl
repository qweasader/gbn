...
if( description )
{
  ...
  script_name("MatrixSSL (GUARD TLS-TK) <= 3.9.5 Side-Channel Attack Vulnerability");
  ...
  script_tag(name:"summary", value:"GUARD TLS-TK (formerly MatrixSSL) is vulnerable to a
  Memory-Cache Side-Channel attack.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The flaw is located in the code that computes signature
  component 's'. In many libraries, the code that performs modular addition does not run in constant
  time, and so a side channel can recover information that can be used to calculate the private key.");
