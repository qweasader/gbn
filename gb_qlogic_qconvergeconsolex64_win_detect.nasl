...
foreach key( key_list ) {
  foreach item( registry_enum_keys( key:key ) ) {
    ...
    if( ! appName || appName !~ "QConvergeConsole x64$" )
      continue;
    ...
    if( vers = registry_get_sz( key:key + item, item:"DisplayVersion" ) ) {
        concluded += '\nDisplayVersion: ' + vers;
      # nb: DisplayVersion is e.g. 5.4.0.86 which is 4.0.86 as the "real" version
    ...
    register_and_report_cpe( app:appName, ver:version, concluded:concluded,
                             base:"cpe:/a:qlogic:qconvergeconsole:", expr:"^([0-9.]+)", insloc:location, regService:"smb-login", regPort:0 );
    exit( 0 );
  }
}
