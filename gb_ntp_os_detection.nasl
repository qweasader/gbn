...
if( ! sys_banner = get_kb_item( "ntp/" + port + "/system_banner" ) )
  exit( 0 );
...
# nb: This should be before the Linux check below because the Meinberg LANTIME devices are also
# exposing a Linux banner but we want to register the LANTIME Firmware before...
if( full_banner ) {
...
# https://en.wikipedia.org/wiki/Darwin_(operating_system)#Release_history
  ...
  if( ! isnull( version[1] ) ) {
    os_register_and_report( os:"Dell EMC Isilon OneFS", version:version[1], cpe:"cpe:/o:emc:isilon_onefs", banner_type:BANNER_TYPE, banner:sys_banner, port:port, proto:"udp", desc:SCRIPT_DESC, runs_key:"unixoide" );
  } else {
    os_register_and_report( os:"Dell EMC Isilon OneFS", cpe:"cpe:/o:emc:isilon_onefs", banner_type:BANNER_TYPE, banner:sys_banner, port:port, proto:"udp", desc:SCRIPT_DESC, runs_key:"unixoide" );
  }
}
