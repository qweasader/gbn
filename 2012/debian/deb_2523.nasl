...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-2523-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-2523-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2012/DSA-2523-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-2523");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'globus-gridftp-server' package(s) announced via the DSA-2523-1 advisory.");
...
if(release == "DEB6") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"globus-gridftp-server-dbg", ver:"3.23-1+squeeze1", rls:"DEB6"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"globus-gridftp-server-progs", ver:"3.23-1+squeeze1", rls:"DEB6"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"libglobus-gridftp-server-dev", ver:"3.23-1+squeeze1", rls:"DEB6"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"libglobus-gridftp-server0", ver:"3.23-1+squeeze1", rls:"DEB6"))) {
    report += res;
  }
