...
if(description)
{
  ...
  script_name("PHP Booking Calendar <= 10e XSS Vulnerability - Active Check");
  ...
  script_tag(name:"summary", value:"PHP Booking Calendar is prone to a cross-site scripting (XSS)
  ...
  script_tag(name:"affected", value:"PHP Booking Calendar 10e and probably prior.");
...
foreach dir( make_list_unique( "/booking_calendar", "/cal", http_cgi_dirs( port:port ) ) ) {
  ...
  res = http_get_cache( port:port, item:dir + "/details_view.php" );
  if( res !~ "^HTTP/1\.[01] 200" || "Booking Calendar" >!< res )
    continue;
