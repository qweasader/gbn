...
if (description)
{
  ...
  script_name("ASP CMS Information Disclosure Vulnerability (Jun 2012) - Active Check");
  ...
  script_tag(name:"impact", value:"An attacker can exploit this issue to gain access to the
  database file stored on the web server. Information harvested may aid in launching further
  attacks.");
...
foreach dir( make_list_unique( "/cms", http_cgi_dirs( port:port ) ) ) {
  ...
  if( http_vuln_check( port:port, url:url, pattern:"Jet DB", check_header:TRUE, icase:FALSE ) ) {
