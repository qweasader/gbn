...
if(description)
{
  ...
  script_name("JAG (Just Another Guestbook) <= 1.14 Information Disclosure Vulnerability");

  script_category(ACT_GATHER_INFO);

  script_copyright("Copyright (C) 2010 Greenbone Networks GmbH");
  script_family("Web application abuses");
  script_dependencies("gb_jag_http_detect.nasl");
  script_require_ports("Services/www", 80);
  script_mandatory_keys("xs4all/jag/http/detected");

  script_tag(name:"summary", value:"JAG is prone to an information disclosure vulnerability.");

  script_tag(name:"vuldetect", value:"Sends a crafted HTTP GET request and checks the response.");

  script_tag(name:"insight", value:"The flaw is caused by improper restrictions on the
  'database.sql' file. By sending a direct request, this can exploited to download the backup
  database.");

  script_tag(name:"impact", value:"Successful exploitation will allow remote attackers to download
  the backup database and obtain sensitive information.");
...
url = dir + "/database.sql";

if (http_vuln_check(port: port, url: url, pattern: "create table guestbook", icase: FALSE)) {
