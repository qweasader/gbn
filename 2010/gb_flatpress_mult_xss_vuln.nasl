...
if(description)
{
  ...
  script_name("FlatPress < 0.909.1 Multiple XSS Vulnerabilities");
  ...
  script_tag(name:"summary", value:"FlatPress is prone to multiple cross-site scripting (XSS)
  vulnerabilities.");

  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host.");

  script_tag(name:"insight", value:"The flaws are due to an error in 'contact.php', 'login.php' and
  'search.php' that fails to sufficiently sanitize user-supplied data via the PATH_INFO.");

  script_tag(name:"impact", value:"Successful exploitation will let the remote attacker to execute
  arbitrary web script or HTML code in a user's browser session in the context of an affected site.");

  script_tag(name:"affected", value:"FlatPress version 0.909 and prior.");

  script_tag(name:"solution", value:"Update to version 0.909.1 or later.");
...
if (version_is_less_equal(version: version, test_version: "0.909")) {
  report = report_fixed_ver(installed_version: version, fixed_version: "0.909.1", install_path: location);
