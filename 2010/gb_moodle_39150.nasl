...
if (description)
{
  ...
  script_name("Moodle 1.8.x < 1.8.12, 1.9.x < 1.9.8 Multiple Vulnerabilities");
  ...
  restrictions, obtain sensitive information, perform unauthorized actions, compromise the
  application, access or modify data, or exploit latent vulnerabilities in the underlying database.
  Other attacks may also be possible.");

  script_tag(name:"affected", value:"Moodle versions 1.8.x prior to 1.8.12 and 1.9.x prior to
  1.9.8.");
...
if(vers =~ "^1\.8" && version_is_less(version:vers, test_version:"1.8.12")) {
  report = report_fixed_ver(installed_version:vers, fixed_version:"1.8.12");
  security_message(port:port, data:report);
  exit(0);
}

else if(vers =~ "^1\.9" && version_is_less(version:vers, test_version:"1.9.8")) {
