...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-970-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-970-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2006/DSA-970-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-970");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'kronolith' package(s) announced via the DSA-970-1 advisory.");
...
if(release == "DEB3.1") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"kronolith", ver:"1.1.4-2sarge1", rls:"DEB3.1"))) {
    report += res;
  }
