...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-849-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-849-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2005/DSA-849-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-849");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'shorewall' package(s) announced via the DSA-849-1 advisory.");
...
if(release == "DEB3.1") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"shorewall", ver:"2.2.3-2", rls:"DEB3.1"))) {
    report += res;
  }
