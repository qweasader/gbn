...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-655-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-655-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2005/DSA-655-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-655");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'zhcon' package(s) announced via the DSA-655-1 advisory.");
...
if(release == "DEB3.0") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"zhcon", ver:"1:0.2-4woody3", rls:"DEB3.0"))) {
    report += res;
  }
