...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-969-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-969-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2006/DSA-969-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-969");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'scponly' package(s) announced via the DSA-969-1 advisory.");
...
if(release == "DEB3.1") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"scponly", ver:"4.0-1sarge1", rls:"DEB3.1"))) {
    report += res;
  }
