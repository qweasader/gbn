...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-681-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-681-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2005/DSA-681-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-681");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'synaesthesia' package(s) announced via the DSA-681-1 advisory.");
...
if(release == "DEB3.0") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"synaesthesia", ver:"2.1-2.1woody3", rls:"DEB3.0"))) {
    report += res;
  }
