...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-654-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-654-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2005/DSA-654-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-654");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'enscript' package(s) announced via the DSA-654-1 advisory.");
...
if(release == "DEB3.0") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"enscript", ver:"1.6.3-1.3", rls:"DEB3.0"))) {
    report += res;
  }
