...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-945-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-945-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2006/DSA-945-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-945");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'antiword' package(s) announced via the DSA-945-1 advisory.");
...
if(release == "DEB3.0") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"antiword", ver:"0.32-2woody0", rls:"DEB3.0"))) {
    report += res;
  }
