...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-983-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-983-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2006/DSA-983-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-983");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'pdftohtml' package(s) announced via the DSA-983-1 advisory.");
...
if(release == "DEB3.1") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"pdftohtml", ver:"0.36-11sarge2", rls:"DEB3.1"))) {
    report += res;
  }
