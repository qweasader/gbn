...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-772-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-772-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2005/DSA-772-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-772");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'apt-cacher' package(s) announced via the DSA-772-1 advisory.");
...
if(release == "DEB3.1") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"apt-cacher", ver:"0.9.4sarge1", rls:"DEB3.1"))) {
    report += res;
  }
