...
if(description)
{
  ...
  script_name("Debian: Security Advisory (DSA-728-1)");
  ...
  script_xref(name:"Advisory-ID", value:"DSA-728-1");
  script_xref(name:"URL", value:"https://www.debian.org/security/2005/DSA-728-1");
  script_xref(name:"URL", value:"https://security-tracker.debian.org/tracker/DSA-728");

  script_tag(name:"summary", value:"The remote host is missing an update for the Debian 'qpopper' package(s) announced via the DSA-728-1 advisory.");
...
if(release == "DEB3.0") {
  ...
  if(!isnull(res = isdpkgvuln(pkg:"qpopper", ver:"4.0.4-2.woody.5", rls:"DEB3.0"))) {
    report += res;
  }

  if(!isnull(res = isdpkgvuln(pkg:"qpopper-drac", ver:"4.0.4-2.woody.5", rls:"DEB3.0"))) {
    report += res;
  }
