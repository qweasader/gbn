...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.54789");
  ...
  script_tag(name:"insight", value:"Various vulnerabilities were found and fixed in Mozilla-based products,
ranging from a potential buffer overflow and temporary files disclosure to
anti-spoofing issues.");
  script_tag(name:"solution", value:"All Mozilla users should upgrade to the latest version:
...
'>=mail-client/mozilla-thunderbird-bin-0.9'");
  ...
  script_xref(name:"URL", value:"http://www.securityspace.com/smysecure/catid.html?in=GLSA%20200501-03");
  script_xref(name:"URL", value:"http://bugs.gentoo.org/show_bug.cgi?id=76112");
  script_xref(name:"URL", value:"http://bugs.gentoo.org/show_bug.cgi?id=68976");
  script_xref(name:"URL", value:"http://bugs.gentoo.org/show_bug.cgi?id=70749");
  script_xref(name:"URL", value:"http://isec.pl/vulnerabilities/isec-0020-mozilla.txt");
  script_xref(name:"URL", value:"http://broadcast.ptraced.net/advisories/008-firefox.thunderbird.txt");
  script_xref(name:"URL", value:"http://secunia.com/advisories/13144/");
