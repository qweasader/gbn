...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.54809");
  ...
  script_tag(name:"insight", value:"Buffer overflow vulnerabilities, which could lead to arbitrary code
execution, have been found in the handling of IPv6 addresses as well as in
the SPA authentication mechanism in Exim.");
  script_tag(name:"solution", value:"All Exim users should upgrade to the latest version:

    # emerge --sync
    # emerge --ask --oneshot --verbose '>=mail-mta/exim-4.43-r2'");

  script_xref(name:"URL", value:"http://www.securityspace.com/smysecure/catid.html?in=GLSA%20200501-23");
  script_xref(name:"URL", value:"http://bugs.gentoo.org/show_bug.cgi?id=76893");
