...
if (description)
{
  ...
  script_name("Butterfly Organizer <= 2.0.1 Multiple Vulnerabilities - Active Check");
  ...
  script_tag(name:"summary", value:"Butterfly Organizer is prone to multiple cross-site scripting
  (XSS) and an SQL injection (SQLi) vulnerability because the application fails to sufficiently
  ...
  script_tag(name:"affected", value:"Butterfly Organizer version 2.0.1 and probably prior.");
...
foreach dir (make_list_unique("/organizer", http_cgi_dirs(port: port))) {
  ...
  url = dir + "/view.php?id=-1+union+select+0x53514c2d496e6a656374696f6e2d54657374,2,3,4,5,6,7,8,9,10+from+test_category&mytable=test_category";
