...
if(description)
{
  ...
  script_name("Free PHP Petition Signing Script SQLi Vulnerability (Mar 2009) - Active Check");
...
foreach dir (make_list_unique("/petition", http_cgi_dirs(port: port))) {
  ...
  res = http_get_cache(port: port, item: dir + "/signing_system-admin/index.php");
  if (!res || res !~ "^HTTP/1\.[01] 200" || !egrep(pattern: "Signing System", string: res) ||
      !egrep(pattern: "Script by Rediscussed\.com", string: res))
    continue;

  url = dir + "/signing_system-admin/index.php";

  data = "username=%27%20or%20%27%201=1&password=";
