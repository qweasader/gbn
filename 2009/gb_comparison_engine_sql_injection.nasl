...
if(description)
{
  ...
  script_name("Comparison Engine Power <= 1.0 SQLi Vulnerability - Active Check");
  ...
  script_tag(name:"summary", value:"Comparison Engine Power is prone to an SQL injection (SQLi)
  ...
  script_tag(name:"affected", value:"Comparison Engine Power version 1.0 and probably prior.");
...
foreach dir (make_list_unique("/comparisonengine", "/compare", http_cgi_dirs(port: port))) {
  ...
  if (http_vuln_check(port: port, url: url, pattern: "O:p:e:n:V:A:S")) {
