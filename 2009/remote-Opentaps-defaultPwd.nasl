...
if(description)
{
  ...
  script_name("Opentaps ERP + CRM Default Credentials (HTTP)");
  script_category(ACT_ATTACK);
  script_copyright("Copyright (C) 2009 Christian Eric Edjenguele");
  script_family("Default Accounts");
  script_dependencies("remote-detect-Opentaps_ERP_CRM.nasl", "gb_default_credentials_options.nasl");
  script_mandatory_keys("OpentapsERP/installed");
  script_exclude_keys("default_credentials/disable_default_account_checks");

  script_tag(name:"summary", value:"Opentaps ERP + CRM is using known default credentials.");
