...
if (description)
{
  ...
  script_name("Beerwin's PhpLinkAdmin <= 1.0 Multiple Vulnerabilities - Active Check");
  ...
  script_tag(name:"impact", value:"A successful exploit may allow an attacker to execute malicious
  code within the context of the webserver process, compromise the application, access or modify
  data, or exploit latent vulnerabilities in the underlying database.");

  script_tag(name:"affected", value:"Beerwin's PhpLinkAdmin version 1.0 and probably prior.");
...
foreach dir (make_list_unique("/phplinkadmin", http_cgi_dirs(port: port))) {
  ...
  res = http_get_cache(port: port, item: dir + "/edlink.php");
  if (!res || res  !~ "^HTTP/1\.[01] 200")
    continue;

  url = dir + "/edlink.php?linkid=-1%27%20union%20all%20select%201,2,3,4,0x53514c2d496e6a656374696f6e2d54657374%27--";
