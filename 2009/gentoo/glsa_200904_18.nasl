...
if(description)
{
  ...
  script_tag(name:"insight", value:"Two errors in udev allow for a local root compromise and a Denial of
    Service.");
  script_tag(name:"solution", value:"All udev users should upgrade to the latest version:

    # emerge --sync
    # emerge --ask --oneshot --verbose '>=sys-fs/udev-124-r2'");

  script_xref(name:"URL", value:"http://www.securityspace.com/smysecure/catid.html?in=GLSA%20200904-18");
  script_xref(name:"URL", value:"http://bugs.gentoo.org/show_bug.cgi?id=266290");
