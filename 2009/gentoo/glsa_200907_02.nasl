...
if(description)
{
  ...
  script_tag(name:"insight", value:"Two vulnerabilities in ModSecurity might lead to a Denial of Service.");
  script_tag(name:"solution", value:"All ModSecurity users should upgrade to the latest version:

    # emerge --sync
    # emerge --ask --oneshot --verbose '>=www-apache/mod_security-2.5.9'");

  script_xref(name:"URL", value:"http://www.securityspace.com/smysecure/catid.html?in=GLSA%20200907-02");
  script_xref(name:"URL", value:"http://bugs.gentoo.org/show_bug.cgi?id=262302");
