...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.63557");
  ...
  script_tag(name:"insight", value:"A buffer overflow in TMSNC might lead to the execution of arbitrary code
when processing an instant message.");
  script_tag(name:"solution", value:"Since TMSNC is no longer maintained, we recommend that users unmerge the
vulnerable package and switch to another console-based MSN client such as
CenterIM or Pebrot:

    # emerge --unmerge 'net-im/tmsnc'");

  script_xref(name:"URL", value:"http://www.securityspace.com/smysecure/catid.html?in=GLSA%20200903-26");
  script_xref(name:"URL", value:"http://bugs.gentoo.org/show_bug.cgi?id=229157");
