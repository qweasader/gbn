...
if(description)
{
  ...
  script_tag(name:"insight", value:"libwmf bundles an old GD version which contains a 'use-after-free'
    vulnerability.");
  script_tag(name:"solution", value:"All libwmf users should upgrade to the latest version which no longer
    builds the GD library:

    # emerge --sync
    # emerge --ask --oneshot --verbose '>=media-libs/libwmf-0.2.8.4-r3'");

  script_xref(name:"URL", value:"http://www.securityspace.com/smysecure/catid.html?in=GLSA%20200907-01");
  script_xref(name:"URL", value:"http://bugs.gentoo.org/show_bug.cgi?id=268161");
