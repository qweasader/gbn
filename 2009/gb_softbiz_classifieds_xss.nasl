...
if(description)
{
  ...
  script_name("Softbiz Classifieds Script Multiple XSS Vulnerabilities (Mar 2009) - Active Check");
  ...
  script_tag(name:"summary", value:"Softbiz Classifieds Script is prone to multiple cross-site
  scripting (XSS) vulnerabilities because it fails to sufficiently sanitize user-supplied data.");
...
foreach dir(make_list_unique("/", http_cgi_dirs(port:port))) {
  ...
  res = http_get_cache(port: port, item: dir + "/showcategory.php");
  if (!res || res !~ "^HTTP/1\.[01] 200")
    continue;

  url = dir + "/showcategory.php?cid=9type=1&keyword=Pouya&radio=%3E%22%3E%3Cscript%3Ealert(document.cookie);%3C/script%3E";
