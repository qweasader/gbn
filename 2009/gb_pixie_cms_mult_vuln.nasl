...
if (description)
{
  ...
  script_name("Pixie CMS Multiple Vulnerabilities (Mar 2009) - Active Check");
  ...
  script_tag(name:"summary", value:"Pixie CMS is prone to an SQL injection (SQLi) vulnerability and
  a cross-site scripting (XSS) vulnerability because it fails to sufficiently sanitize
  user-supplied data.");
...
foreach dir (make_list_unique("/cms", http_cgi_dirs(port: port))) {
  ...
  if (!res || res !~ "^HTTP/1\.[01] 200" || ("Pixie Powered" >!< res && res !~ "Set-Cookie\s*:\s*bb2_screener_"))
    continue;

  url = dir + "/index.php?s=blog&m=permalink&x=%22%3E%3Cscript%3Ealert(document.cookie)%3C/script%3E";
