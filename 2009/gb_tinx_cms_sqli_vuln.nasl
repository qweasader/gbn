...
if(description)
{
  ...
  script_name("TinX CMS < 3.5.1 SQLi Vulnerability - Active Check");
  ...
  script_tag(name:"summary", value:"TinX CMS is prone to an SQL injection (SQLi) vulnerability
  ...
  script_tag(name:"affected", value:"TinX CMS prior to version 3.5.1.");
...
foreach dir (make_list_unique("/cms", "/tinxcms", http_cgi_dirs(port: port))) {
  ...
  res = http_get_cache(port: port, item: dir + "/system/rss.php");
  if (!res || res !~ "^HTTP/1\.[01] 200")
    continue;

  url = dir + "/system/rss.php?id=-1%20union%20select%201,22222222222,3,4,5,6,7,8,0x53514c2d496e6a656374696f6e2d54657374;";
