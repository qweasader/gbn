...
if (description)
{
  ...
  script_name("Bugs Online <= 2.14 SQLi Vulnerability - Active Check");
  ...
  script_tag(name:"summary", value:"Bugs Online is prone to an SQL injection (SQLi) vulnerability
  ...
  script_tag(name:"affected", value:"Bugs Online version 2.14 and probably prior.");
...
foreach dir( make_list_unique( "/", http_cgi_dirs( port:port ) ) ) {
  ...
  res = http_get_cache( port:port, item:dir + "/help.asp" );
  if( !res || res !~ "^HTTP/1\.[01] 200" )
    continue;

  url = dir + "/help.asp?stype=-999'%20union%20select%201,2,'" + vt_strings["default"] +
              "'%20from%20tblusers%20where%20'1";
