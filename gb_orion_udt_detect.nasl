...
if (description)
{
  ...
  script_name("SolarWinds User Device Tracker (UDT) Detection (HTTP)");
  ...
  script_tag(name:"summary", value:"HTTP based detection of SolarWinds User Device Tracker (UDT).");
...
if ("SolarWinds Platform" >< buf || "SolarWinds Orion" >< buf || "Orion Platform" >< buf) {
  ...
  if (!isnull(udt)) {
    ...
    set_kb_item(name:"solarwinds/udt/detected", value:TRUE);
    set_kb_item(name:"solarwinds/udt/http/detected", value:TRUE);
    ...
    log_message(data:build_detection_report(app:"SolarWinds User Device Tracker (UDT)", version:vers,
                                            install:dir, cpe:cpe,
                                            concluded:udt[0]),
                port:port);
  }
